...
public class LogFormat {
    ...
    public static String formatBorder(String[] segments) {
        ...
        msgBuilder.append(TOP_HORIZONTAL_BORDER).append(WiiPrinter.lineSeparator);
        for (int i = 0; i < nonNullCount; i++) {
            msgBuilder.append(appendVerticalBorder(nonNullSegments[i]));
            if (i != nonNullCount - 1) {
                msgBuilder.append(WiiPrinter.lineSeparator).append(DIVIDER_HORIZONTAL_BORDER)
                        .append(WiiPrinter.lineSeparator);
            } else {
                msgBuilder.append(WiiPrinter.lineSeparator).append(BOTTOM_HORIZONTAL_BORDER);
            }
        }
    ...
    private static String appendVerticalBorder(String msg) {
        ...
        String[] lines = msg.split(WiiPrinter.lineSeparator);
        for (int i = 0, N = lines.length; i < N; i++) {
            if (i != 0) {
                borderedMsgBuilder.append(WiiPrinter.lineSeparator);
            }
            String line = lines[i];
